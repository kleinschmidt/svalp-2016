...
ggplot(curves, aes(x=x, y=p, color=Talker)) +
  geom_line(size=2) +
  scale_y_continuous('Probability of /s/') +
  scale_x_continuous("Primary /s/-/sh/ cue\n(Frication frequency centroid, Hz)") +
...
id_particle <- function(x, p) {
  ...
  # compute likelihood
  sapply(x, function(xx) normalize(dnorm(xx, mean=p$mu, sd=p$sigma, log=FALSE) * p$theta)[1])
...
```{r okay_samples, fig.width=4, fig.height=3, out.width="50%"}
...
## belief updating animation for FLAT prior and MISINFORMED
