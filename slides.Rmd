...
## force png for fig.show='animate'
...
* VOT drawn from bimodal distribution

```{r exposure-build-up, fig.show='animate'}
...
ggplot(one_sub, aes(x=vot, group=trials_ten,
                    frame=trials_ten, cumulative=TRUE)) +
  geom_bar(binwidth=10) +
  geom_point(aes(y=-5), position=position_jitter(w=5, h=5), alpha=0.2) +
...
```{r dist-learning-schematic, fig.width=3, fig.height=3}
...
```{r boundary-violin-plots, fig.widt=6.5, fig.height=4.5}
...
```{r lhood-fcn-update-animation, fig.show='animate'}
...
```{r class-fcn-update-animation, fig.show='animate'}
