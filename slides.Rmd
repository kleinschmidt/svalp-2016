...
r_particle_prior <- function(mu0 = c(0, 60),
                             mu_sigma = c(60, 60),
                             sigma0 = c(5, 30),
                             sigma_sigma = c(5, 30)){
  mu <- mapply(rnorm, n=1, mean=mu0, sd=mu_sigma)
  sigma <- mapply(rgamma_meansd, n=1, mean=sigma0, sd=sigma_sigma)
  n <- max(sapply(list(mu, sigma), length))
  list(mu=mu, sigma=sigma, theta=rep(1/n, n))
}

n_part <- 100
particles <- lapply(1:n_part, r_particle_prior)
...
```{r belief-updating-animation, fig.show='animate'}
